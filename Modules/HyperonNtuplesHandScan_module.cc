////////////////////////////////////////////////////////////////////////
// Class:       HyperonNtuplesHandScan
// Plugin Type: analyzer (art v3_03_01)
// File:        HyperonNtuplesHandScan_module.cc
//
...
class hyperon::HyperonNtuplesHandScan : public art::EDAnalyzer {
   public:
      explicit HyperonNtuplesHandScan(fhicl::ParameterSet const& p);
      // The compiler-generated destructor is fine for non-base
      // classes without bare pointers or other resource use.

      // Plugins should not be copied or assigned.
      HyperonNtuplesHandScan(HyperonNtuplesHandScan const&) = delete;
      HyperonNtuplesHandScan(HyperonNtuplesHandScan&&) = delete;
      HyperonNtuplesHandScan& operator=(HyperonNtuplesHandScan const&) = delete;
      HyperonNtuplesHandScan& operator=(HyperonNtuplesHandScan&&) = delete;
...
hyperon::HyperonNtuplesHandScan::HyperonNtuplesHandScan(fhicl::ParameterSet const& p)
   : EDAnalyzer{p},
...
void hyperon::HyperonNtuplesHandScan::analyze(art::Event const& e)
{
   ...
   if(!e.getByLabel(f_Generator.get<std::string>("GeneratorModuleLabel","generator"),Handle_MCTruth))  
      t_IsData = true;      
   ...
   if(!t_IsData && f_GetGeneratorInfo){
   ...
   if(f_GetRecoInfo){
      ...
      SubModuleReco* Reco_SM = new SubModuleReco(e,t_IsData,f_Reco);
      Reco_SM->PrepareInfo();
   ...
   if(!t_IsData){
      ...
      for(size_t i_w=0;i_w<f_WeightLabels.size();i_w++){
         ...
         if(!e.getByLabel(f_WeightLabels.at(i_w),Handle_EventWeight)) 
            throw cet::exception("HyperonNtuplesHandScan") << "No EventWeight Found!" << std::endl;

         art::fill_ptr_vector(Vect_EventWeight,Handle_EventWeight);

         if(!Vect_EventWeight.size())
            throw cet::exception("HyperonNtuplesHandScan") << "Weight vector empty!" << std::endl;

         if(Vect_EventWeight.size() != (size_t)t_NMCTruths)
            throw cet::exception("HyperonNtuplesHandScan") << "Weight vector size != NMCTruths" << std::endl;
         ...
         for(size_t i_tr=0;i_tr<Vect_EventWeight.size();i_tr++){       
            ...
            for(it = theWeights.begin(); it != theWeights.end();it++){
               ...
               if(!dial_found){
                  if(i_tr != 0)
                     throw cet::exception("HyperonNtuplesHandScan") << "Malformed systematics weight vectors" << std::endl;
...
void hyperon::HyperonNtuplesHandScan::FinishEvent(){
...
void hyperon::HyperonNtuplesHandScan::beginJob(){
   ...
   OutputTree->Branch("IsData",&t_IsData);
...
void hyperon::HyperonNtuplesHandScan::endJob()
{
   MetaTree->Fill();
}

void hyperon::HyperonNtuplesHandScan::beginSubRun(const art::SubRun& sr)
{
...
void hyperon::HyperonNtuplesHandScan::endSubRun(const art::SubRun& sr){}

DEFINE_ART_MODULE(hyperon::HyperonNtuplesHandScan)
