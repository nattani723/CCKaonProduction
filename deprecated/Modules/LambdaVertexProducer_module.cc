...
class cckaon::LambdaVertexProducer : public art::EDProducer {
   public:
...
cckaon::LambdaVertexProducer::LambdaVertexProducer(fhicl::ParameterSet const& p)
   : EDProducer{p},
...
void cckaon::LambdaVertexProducer::produce(art::Event& e)
{
   ...
   SubModuleReco* Reco_SM = new cckaon::SubModuleReco(e,false,f_Reco);
   Reco_SM->PrepareInfo();
   ...
   if(f_KaonCheat){
      for(size_t i_tr=0;i_tr<RecoD.TrackPrimaryDaughters.size();i_tr++){
         if(RecoD.TrackPrimaryDaughters.at(i_tr).TrackTrueOrigin == 2 && (RecoD.TrackPrimaryDaughters.at(i_tr).TrackTruePDG == 2212 || RecoD.TrackPrimaryDaughters.at(i_tr).TrackTruePDG == -211)){
            ...
            if(RecoD.TrackPrimaryDaughters.at(i_tr).TrackTruePDG == 2212) std::cout << "Found proton from Kaon decay at index " << tracktouseindex << std::endl;
            if(RecoD.TrackPrimaryDaughters.at(i_tr).TrackTruePDG == -211) std::cout << "Found pi minus from Kaon decay at index " << tracktouseindex << std::endl;
            break;
         }
      }
   }
   ...
   else throw cet::exception("LambdaVertexProducer") << "Please set a track/shower to use, or set KaonCheat to true" << std::endl;
...
DEFINE_ART_MODULE(cckaon::LambdaVertexProducer)
