...
// File:        SecondaryVertexProducer_module.cc
//
...
class SecondaryVertexProducer : public art::EDProducer {
   public:
      explicit SecondaryVertexProducer(fhicl::ParameterSet const& p);
      // The compiler-generated destructor is fine for non-base
      // classes without bare pointers or other resource use.

      // Plugins should not be copied or assigned.
      SecondaryVertexProducer(SecondaryVertexProducer const&) = delete;
      SecondaryVertexProducer(SecondaryVertexProducer&&) = delete;
      SecondaryVertexProducer& operator=(SecondaryVertexProducer const&) = delete;
      SecondaryVertexProducer& operator=(SecondaryVertexProducer&&) = delete;
...
SecondaryVertexProducer::SecondaryVertexProducer(fhicl::ParameterSet const& p)
   : EDProducer{p},
...
void SecondaryVertexProducer::produce(art::Event& e)
{
   ...
   // If KaonCheat is enabled, use truth matching info to select the kaon track
   if(f_KaonCheat){
      hyperon::SubModuleReco* Reco_SM = new hyperon::SubModuleReco(e,false,f_Reco);
      Reco_SM->PrepareInfo();
      //Reco_SM->SetIndices(t_IsSignal,t_IsSignalSigmaZero);
      hyperon::RecoData RecoD = Reco_SM->GetInfo(); 
   ...
   else if(f_TrackIndex == -1) tracktouseindex = 0;
   else tracktouseindex = f_TrackIndex;
   ...
   if(!e.getByLabel(f_PFParticleLabel,Handle_PFParticle)) 
      throw cet::exception("SecondaryVertexProducer") << "No PFParticle Data Products Found!" << std::endl;

   if(!e.getByLabel(f_TrackLabel,Handle_Track)) 
      throw cet::exception("SecondaryVertexProducer") << "No Track Data Products Found!" << std::endl;
   ...
   if(tracktouseindex != -1 && xyz[0] == -1000 && xyz[1] == -1000 && xyz[2] == -1000)
      throw cet::exception("SecondaryVertexProducer") << "Couldn't find a track with the index requested" << std::endl;

    //AddExternalVertex(e,TVector3(xyz[0],xyz[1],xyz[2]));  
...
DEFINE_ART_MODULE(SecondaryVertexProducer)
